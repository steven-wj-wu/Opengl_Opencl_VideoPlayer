...
buffer* transfer_ptr = &buffer_head;
...
void gpu_transfer_muilti_buffer(VideoData* data);
...
int main()
{
    ...
    */
    ...
    int RGBFramesize, YUVFramesize;
    RGBFramesize = av_image_get_buffer_size(AV_PIX_FMT_BGR24, width, height, 1);
    YUVFramesize = av_image_get_buffer_size(AV_PIX_FMT_YUV420P, width, height, 1);
    ...
    buffer_head.tmp_frame_nv12 = av_frame_alloc();
    buffer_head.tmp_frame_yuv = av_frame_alloc();
    ...
    buffer_head.internal_buffer_yuv = (uint8_t*)av_malloc(YUVFramesize * sizeof(uint8_t));
    av_image_fill_arrays(buffer_head.tmp_frame_yuv->data, buffer_head.tmp_frame_yuv->linesize, buffer_head.internal_buffer_yuv, AV_PIX_FMT_YUV420P, width, height, 1);
    ...
    for (int i = 0; i < buffer_num - 1; i++) {
        ...
        current->tmp_frame_nv12 = av_frame_alloc();
        current->tmp_frame_yuv = av_frame_alloc();
        ...
        current->internal_buffer_yuv = (uint8_t*)av_malloc(YUVFramesize * sizeof(uint8_t));
        av_image_fill_arrays(current->tmp_frame_yuv->data, current->tmp_frame_yuv->linesize, current->internal_buffer_yuv, AV_PIX_FMT_YUV420P, width, height, 1);
    ...
    std::thread transfer_write(gpu_transfer_muilti_buffer, &my_video);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (read_ptr->read && stop) {
            ...
            //clEnqueueWriteBuffer(command_queue, gpu_ptr->data, CL_TRUE, 0, height * width * 3 * sizeof(unsigned char), gpu_ptr->frame_buffer->data[0], 0, NULL, NULL);
...
void read_multi_buffer(VideoData* data) {
            ...
            if (write_ptr->image_write) {
                ...
                if (data->packet->stream_index == data->stream_idx) {
                    ...
                    if (!(avcodec_receive_frame(data->codec_ctx, write_ptr->codec_buffer) < 0)) {
                        ...
                        std::cout << "decoede time:" << (end - start) * 1000 << std::endl;
    ...
    std::cout << "avg decode time:" << time / num_frame << std::endl;
...
void gpu_transfer_muilti_buffer(VideoData* data) {
    ...
    while (1) {
        ...
        if (transfer_ptr->codec_buffer == NULL)
        {
            transfer_ptr->tmp_frame_yuv = NULL;
            break;
        }

        if (transfer_ptr->trasfer) {
            int ret;



            if ((ret = av_hwframe_transfer_data(transfer_ptr->tmp_frame_nv12, transfer_ptr->codec_buffer, 0)) < 0) {
                fprintf(stderr, "Error transferring the data to system memory\n");
            }
            sws_scale(data->conv_ctx_yuv, transfer_ptr->tmp_frame_nv12->data, transfer_ptr->tmp_frame_nv12->linesize, 0, data->codec_ctx->height, transfer_ptr->tmp_frame_yuv->data, transfer_ptr->tmp_frame_yuv->linesize);


            /*
            time += (end - start) / 1000;
            if ((end - start) > max_time && (end - start)*1000 < 18)
                max_time = (end - start)*1000;
            if ((end - start) < min_time)
                min_time = (end - start)*1000;
            num_frame++;
            */

            transfer_ptr->trasfer = 0;
            transfer_ptr->gpu_write = 1;
            transfer_ptr = transfer_ptr->next_buffer;

        }
        //std::cout << "AVG read time:" << time / num_frame << std::endl;
        //std::cout << "MAX read time:" << max_time / 1000 << std::endl;
        //std::cout << "MIN read time" << min_time / 1000 << std::endl;
    }
...
void gpu_write_muilti_buffer(VideoData* data) {
    ...
    while (1)
    {
        ...
        if (gpu_ptr->gpu_write && gpu_ptr->read == 0) {
            ...
            sws_scale(data->conv_ctx, gpu_ptr->tmp_frame_yuv->data, gpu_ptr->tmp_frame_yuv->linesize, 0, data->codec_ctx->height, gpu_ptr->frame_buffer->data, gpu_ptr->frame_buffer->linesize);
            ...
            //std::cout << "decoede fps:" << 1 / (end - start) << std::endl;    
